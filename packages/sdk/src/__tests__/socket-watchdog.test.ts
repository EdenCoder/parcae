...
describe("SocketTransport — watchdog", () => {
  ...
  it("stays idle after manual disconnect and re-arms on reconnect", async () => {
    ...
    // reconnect() awaits the connection, which the recovery's engine
    // rebuild rejects; the watchdog re-arms synchronously either way.
    const reconnected = transport.reconnect();
    reconnected.catch(() => {});
    await vi.advanceTimersByTimeAsync(8_100);
    expect(events).toHaveLength(1);
  });
